  ...
  const handleOAuth = async (providerName: string, getProvider: () => any) => {
    try {
      ...
      await signInWithOAuth({
        id: user.uid,
        email: user.email ?? `${user.uid}@oauth.placeholder.com`,
        name: user.displayName || "ผู้ใช้ไม่มีชื่อ",
  ...
  return (
    <div className="flex flex-col gap-3">
      <Button
        type="button"
        variant="outline"
        className="w-full text-foreground/80 hover:bg-slate-50 hover:text-foreground"
        disabled={loadingProvider !== null}
        onClick={() => handleOAuth("Google", () => new GoogleAuthProvider())}
      >
